...
**Why NOT AI Gateway for this project:** AI Gateway adds value for multi-provider routing, budgeting, and load balancing -- features a single-user personal app doesn't need. Direct provider keeps the dependency chain shorter.
...
Each book is a `.md` file in a `library/` directory:
...
rating: 5
genres: ["ficção", "clássico brasileiro"]
cover: "/covers/o-alienista.jpg"
added: "2026-04-15"
finished: "2026-03-20"
---
...
Um dos contos mais brilhantes da literatura brasileira...
```
...
2. Fetch Google Books API first (better metadata, especially for BR books)
3. If no results or missing cover, fallback to Open Library
4. Merge best data from both sources
5. Write to `.md` file

**No SDK needed for either API.** Both are simple REST endpoints. A thin wrapper with `fetch()` is cleaner than pulling in a dependency.
...
The entire value proposition is **Markdown files as the single source of truth**. This means:
...
**Performance at scale:** Reading 1,000 .md files with gray-matter takes <500ms. Cache the parsed results in memory and invalidate on write. This is more than enough for personal use.

## Installation

```bash
# Initialize Next.js project
npx create-next-app@latest dona-flora --typescript --tailwind --eslint --app --src-dir

# Core AI
npm install ai @ai-sdk/react @ai-sdk/anthropic

# Markdown data layer
npm install gray-matter remark remark-html zod
...
```bash
# .env.local
ANTHROPIC_API_KEY=sk-ant-...    # Only secret needed
```

No database URL. No auth secrets. One API key.
...
dona-flora/
  src/
    app/
      ...
      chat/page.tsx               # Librarian chat interface
      books/[slug]/page.tsx       # Individual book detail
      api/
        chat/route.ts             # AI chat endpoint
        books/route.ts            # CRUD for books
        books/search/route.ts     # Google Books / Open Library search
    lib/
      books.ts                    # Read/write .md files, Zod schemas
      book-apis.ts                # Google Books + Open Library fetchers
      agent.ts                    # ToolLoopAgent definition (librarian)
    components/
      ui/                         # shadcn/ui components
      chat/                       # Chat UI components
      library/                    # Book grid, filters, cards
  library/                        # THE DATA: one .md per book
    o-alienista.md
    dom-casmurro.md
    ...
```
