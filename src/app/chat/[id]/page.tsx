...
import { loadChatRecord } from '@/lib/chats/store'
...
export default async function ChatIdPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const record = await loadChatRecord(id, storageContext)
  if (!record) notFound()
  ...
  return (
    <Suspense fallback={null}>
      <ChatShell
        chatId={id}
        initialGenerationStatus={record.chat.generation_status}
        initialLastError={record.chat.last_error}
