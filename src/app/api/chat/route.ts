...
export async function POST(request: NextRequest) {
  ...
  const persistedMessages = messages as unknown as LibrarianMessage[]
    ...
    if (storageContext) {
      try {
        await saveChat({
          ...
          lastError:
            err instanceof Error ? err.message : 'A Dona Flora não conseguiu concluir a resposta.',
          messages: persistedMessages,
          storageContext,
        })
      } catch (saveErr) {
        console.error('[API] POST /api/chat save error state failed:', saveErr)
      }
    }
