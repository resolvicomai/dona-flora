...
describe('POST /api/chat — streamText wiring', () => {
  ...
  it('onFinish calls saveChat with chatId and messages', async () => {
    const inboundMessages = validMessages()
    await POST(makeRequest({ chatId: 'meu-chat', messages: inboundMessages }))
    ...
    expect(mockedSaveChat).toHaveBeenNthCalledWith(1, {
      chatId: 'meu-chat',
    ...
    expect(mockedSaveChat).toHaveBeenLastCalledWith({
      chatId: 'meu-chat',
