...
import { ChatFrontmatterSchema, type ChatFrontmatter, type ChatGenerationStatus } from './schema'
...
export async function updateChatMetadata({
  ...
  const file = matter.stringify(parsedFile.content, withoutUndefined(next))
  await fs.writeFile(filepath, file, 'utf-8')
  return next
}

const CHAT_GENERATION_TIMEOUT_MS = 10 * 60 * 1000
const STALE_GENERATION_ERROR =
  'A resposta anterior foi interrompida antes de terminar. Tente novamente.'

export function applyEffectiveGenerationState(chat: ChatFrontmatter): ChatFrontmatter {
  if (chat.generation_status !== 'generating') {
    return chat
  }
  ...
  if (Date.now() - updatedAt <= CHAT_GENERATION_TIMEOUT_MS) {
    return chat
  }
  ...
  return {
    ...
    last_error: chat.last_error ?? STALE_GENERATION_ERROR,
  }
}
...
export async function loadChatRecord(
  chatId: string,
  storageContext?: StorageContext,
): Promise<{ chat: ChatFrontmatter; messages: LibrarianMessage[] } | null> {
  const filepath = path.join(getChatsDir(storageContext), `${chatId}.md`)
  let raw: string
  try {
    raw = await fs.readFile(filepath, 'utf-8')
  ...
  try {
    ...
    return {
      chat: applyEffectiveGenerationState(parsed.data),
...
function normalizeChatFrontmatter(data: Record<string, unknown>) {
  return {
    ...
    generation_status:
      data?.generation_status === 'generating' || data?.generation_status === 'error'
